        ...
        >
            <v-card-text>
                <div class="flex justify-between ga-3 mb-3">
                    <div>
                        <div class="text-caption">{{ comment.username }}</div>
                        <div class="text-medium-emphasis">
                            {{ new Date(comment.createdAt).toLocaleDateString() }}
                        </div>
                    </div>
                    <v-progress-circular :model-value="ratingPercentage(comment.rating)" :size="40">
                        {{  comment.rating }}
                    </v-progress-circular>
                </div>
                <div v-html="comment.message" class="text-body-large" />
            </v-card-text>
        </v-card>
    </div>
</template>
